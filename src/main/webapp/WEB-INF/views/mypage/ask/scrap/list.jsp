...
$(document).ready(function(){
	...
	$("#btnDelete").click(function() {
		...
		action_popup.confirm("선택한 글을 스크랩 해제하시겠습니까?\n해당 작업은 되돌릴 수 없습니다.", function(result) {
			...
			if(result == true){
				location.href="/mypage/ask/scrap/delete?scrapNo="+delchk;
			} else {
				return false;
			}
		})
	...
	<div class="col-md-9" style="height: 500px;">
		<div class="tablearea">
			...
			<tbody>
				...
				<tr>
					<td><input type="checkbox" id="${ask.SCRAP_NO }" value="${ask.SCRAP_NO }" class="chk" /></td>
					<td>${ask.ASK_NO }</td>
					<td><a href="<%=request.getContextPath() %>/ask/detail?askNo=${ask.ASK_NO }">${ask.ASK_TITLE }</a></td>
					<td>${ask.ASK_HIT }</td>
					<fmt:formatDate value="${ask.ASK_DATE }" pattern="yyyyMMdd" var="timeStr" />
